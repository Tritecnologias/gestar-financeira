...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await db.cliente.update({ where: { id }, data: { codigo: codigo?.trim().toUpperCase(), nome: nome.trim(), email: email || null, telefone: telefone || null, documento: documento || null, endereco: endereco || null } });
