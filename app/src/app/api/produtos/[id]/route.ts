...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await db.produto.update({ where: { id }, data: { codigo: codigo?.trim(), nome: nome.trim(), tipo: tipo || null, unidade: unidade || null, precoVenda: precoVenda ? parseFloat(precoVenda) : null, precoCusto: precoCusto ? parseFloat(precoCusto) : null, categoria: categoria || null } });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { ({ db } = await requireSession()); } catch { return NextResponse.json({ error: "Não autorizado" }, { status: 401 }); }
  const { id } = await params;
  try { await db.produto.update({ where: { id }, data: { ativo: false } }); return NextResponse.json({ ok: true }); }
