...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await db.pessoa.update({ where: { id }, data: { codigo: codigo?.trim(), nome: nome.trim(), cargo: cargo || null, departamento: departamento || null, email: email || null, telefone: telefone || null, documento: documento || null, dataAdmissao: dataAdmissao ? new Date(dataAdmissao) : null, salario: salario ? parseFloat(salario) : null } });
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { ({ db } = await requireSession()); } catch { return NextResponse.json({ error: "Não autorizado" }, { status: 401 }); }
  const { id } = await params;
  try { await db.pessoa.update({ where: { id }, data: { ativo: false } }); return NextResponse.json({ ok: true }); }
